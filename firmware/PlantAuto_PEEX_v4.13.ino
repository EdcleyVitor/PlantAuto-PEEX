/*
  PlantAuto PEEX - Firmware ESP32 BLE v4.13
  ...
  Comandos recebidos pelo app:
    ...
    @VERSION                  -> responde "@VERSION:4.13" (versão do firmware)
...
class CharCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic* characteristic) {
    ...
    if (comando.startsWith("@TIME:")) {
      ...
      // Versão do firmware (v4.13): o app mostra para confirmar a versão gravada
      enviar("@VERSION:4.13");
...
void setup() {
  ...
  Serial.println("PlantAuto PEEX v4.13 pronto. Nome BLE: " + nomeBLE);
...
void loop() {
  ...
  // Rega automática (só depois de 5s do boot, evitando acionar o relé na hora de energizar)
  if (millis() >= 5000) {
